...
namespace Trackwane.AccessControl.Tests.Behavior.Engine.Users.Queries
{
    internal class Find_By_Key_Tests : Scenario
    {
        ...
        {
            var user = EngineHost.ExecutionEngine.Query<FindByKey>(ApplicationKey).Execute(USER_1_IN_ORGANIZATION_A_KEY);
        ...
        {
            var user = EngineHost.ExecutionEngine.Query<FindByKey>(ApplicationKey).Execute(Guid.NewGuid().ToString());
            user.ShouldBeNull();
        }
    }
}
